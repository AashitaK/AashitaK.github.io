...
- A [Python package](https://github.com/AashitaK/nyt-comments) that includes three main functions to perform three distinct tasks involving the retrieval of comments' and articles' from New York Times as ready-to-use dataset for data science/machine learning projects:
  1. The main function **get_dataset** returns two dataframes - one each for the articles and the respective comments. The retrieval can be customized based on a number of parameters such as a specific timeline for the articles, search keywords, filter queries based on options such as the week of the day, the word count of the articles, source, etc., the maximum limit on the number of comments or articles or both, sorting of the articles chronologically based on either the newest or oldest articles, option to save the data as two csv files, etc.   
